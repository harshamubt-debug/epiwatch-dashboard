...
st.caption("Powered by Grok AI. Ask questions in plain English — the AI fetches live data and answers.")
...
if not GROK_API_KEY:
    st.warning("⚠️ Add your Grok API key in Streamlit Secrets to enable AI features.")
    st.code('GROK_API_KEY = "xai-your-key-here"', language="toml")
    st.info("Get a free key at [console.x.ai](https://console.x.ai)")
    st.stop()
...
TOP 10 COUNTRIES: {json.dumps(context['top_10_countries_by_cases'])}
TOP 5 RISK COUNTRIES: {json.dumps(context['top_5_risk_countries_today'])}
ACTIVE ALERTS: {json.dumps(context['active_alerts'])}
INDIA DISEASE BURDEN: {json.dumps(context['india_disease_burden_top5'])}
Answer questions about global and India-specific disease surveillance. Be precise with numbers. Max 400 words. For surveillance/research only."""
...
if user_input:
    ...
    with st.chat_message("assistant", avatar="🦠"):
        with st.spinner("Analyzing live epidemiological data..."):
            ...
            response = call_grok(st.session_state["messages"][-10:], system)
...
if gen_report:
    with st.spinner(f"Generating {report_type}..."):
        context = build_context()
        system  = get_system_prompt(context)
        prompt  = f"Generate a professional {report_type} with sections: Executive Summary, Key Findings, High-Priority Situations, Trends & Projections, Recommendations. Date: {context['timestamp']}. For surveillance purposes only."
        report  = call_grok([{"role": "user", "content": prompt}], system)
    st.markdown(report)
