...
with col_feed:
    ...
    for alert in filtered:
        ...
        st.markdown(f"""
        <div style="border-left:3px solid {border_color};background:{bg};padding:10px 14px;border-radius:0 8px 8px 0;margin-bottom:8px">
            <div style="margin-bottom:5px">{sev_badge}{disease_badge}</div>
            <div style="font-size:0.85rem;font-weight:600;color:#e6edf3;margin-bottom:4px">{title}</div>
            <div style="font-size:0.76rem;color:#8b949e;line-height:1.5">{summary}</div>
            <div style="font-size:0.72rem;color:#6e7681;margin-top:6px">
                📡 {source} &nbsp;|&nbsp; 🕐 {date} &nbsp;|&nbsp;
                <a href="{link}" target="_blank" style="color:#58a6ff;text-decoration:none">Read more →</a>
            </div>
        </div>""", unsafe_allow_html=True)
